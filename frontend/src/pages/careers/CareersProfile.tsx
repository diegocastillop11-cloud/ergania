...
import {
  ...
  Linkedin, Copy, CheckCircle2, Sparkles, Upload, Plus, X, Pencil,
} from 'lucide-react'
...
// ── Pestañas de perfiles (multi-perfil) ──────────────────────────────────────
...
function PerfilTabs() {
  ...
  const remove = (p: Perfil) => {
    if (!window.confirm(`¿Eliminar el perfil "${p.nombre}"? Se borrarán sus datos y su CV. Esta acción no se puede deshacer.`)) return
