...
export default function SubscriptionBanner({ sub }: Props) {
  ...
  // MP (Preapproval) y PayPal (Subscriptions) cobran solos — no hay renovación
  // manual que recordar para ningún proveedor.
  const paymentSuspended = sub.status === 'active' && sub.paymentSuspended
  const pendingInTrial = sub.status === 'pending_payment' && sub.daysLeft !== null && sub.daysLeft > 0
  if (sub.loading || (sub.status === 'active' && !paymentSuspended)) return null
  if ((sub.status === 'trial' || pendingInTrial) && dismissed) return null
  ...
  // Cobro automático de MP falló y se agotaron los reintentos: pedir actualizar la tarjeta
  if (paymentSuspended) {
    return (
      <div className="flex flex-col gap-1 mb-4">
        <div className="flex items-center gap-3 bg-red-950/60 border border-red-700/40 rounded-xl px-4 py-2.5">
          <AlertTriangle size={15} className="text-red-400 shrink-0" />
          <p className="text-sm text-red-200 flex-1">
            No pudimos cobrar tu suscripción — actualiza tu método de pago para no perder acceso
          </p>
          <button
            onClick={handleSubscribe}
            disabled={loadingCheckout}
            className="flex items-center gap-1.5 text-xs font-semibold text-white bg-red-600 hover:bg-red-500 disabled:opacity-60 px-3 py-1.5 rounded-lg transition-colors"
          >
