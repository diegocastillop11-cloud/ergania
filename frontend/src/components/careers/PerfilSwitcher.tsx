  ...
  return (
    <div className="relative px-3 pt-3">
      <button
        onClick={() => setOpen(o => !o)}
        disabled={busy}
        className="w-full flex items-center gap-2 px-2.5 py-2 bg-[var(--bg-surface-alt)] hover:bg-gray-700 border border-[var(--border-alt)] rounded-lg text-xs text-[var(--text-primary)] disabled:opacity-60 transition-colors"
        title={t('perfilSwitcher.changeProfileTitle')}
      >
        <UserCircle2 size={14} className="text-blue-400 shrink-0" />
        <span className="flex-1 text-left truncate font-medium">{active?.nombre || t('perfilSwitcher.defaultLabel')}</span>
        <ChevronDown size={13} className="text-[var(--text-muted)] shrink-0" />
      </button>
      {open && (
        <>
          <div className="fixed inset-0 z-10" onClick={() => setOpen(false)} />
          <div className="absolute left-3 right-3 mt-1 bg-[var(--bg-surface-alt)] border border-[var(--border-alt)] rounded-lg shadow-xl z-20 overflow-hidden">
            {perfiles.map(p => (
              <button
                key={p.id}
                onClick={() => switchTo(p)}
                className={`w-full text-left px-3 py-2 text-xs transition-colors ${
                  p.isActive ? 'bg-blue-600 text-[var(--text-primary)]' : 'text-[var(--text-secondary)] hover:bg-gray-700'
                }`}
              >
                {p.nombre}
              </button>
